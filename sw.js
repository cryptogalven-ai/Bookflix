...
self.addEventListener('fetch',event=>{const u=new URL(event.request.url);if(event.request.method!=='GET')return;if(u.origin===location.origin){event.respondWith((async()=>{const cached=await caches.match(event.request);try{const network=await fetch(event.request);if(network.ok){if(isPage(network))return await cachePage(await caches.open(CACHE),event.request,network);const copy=network.clone();caches.open(CACHE).then(c=>c.put(event.request,copy))}return network}catch{return cached||caches.match('/index.html')}})());return}if(isOpenLibrary(u))event.respondWith(caches.match(event.request).then(cached=>{const network=fetch(event.request).then(r=>{if(r.ok){const copy=r.clone();caches.open(CACHE).then(c=>c.put(event.request,copy))}return r}).catch(()=>cached);return cached||network}));});
