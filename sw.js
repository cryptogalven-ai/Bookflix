const CACHE='bookflix-shell-v4';
const SHELL=['/','/index.html','/manifest.json','/icon.svg','/enhancements.js'];
...
async function enhancedResponse(request,response){
  ...
  try{
    ...
    const injected=text.replace('</body>','<script src="/enhancements.js"></script></body>');
  ...
  if(u.origin===location.origin){
    event.respondWith((async()=>{
      ...
      try{
        ...
        if(network.ok){const copy=network.clone();caches.open(CACHE).then(c=>c.put(event.request,copy));}
        return await enhancedResponse(event.request,network);
      }catch{return cached||caches.match('/index.html')}
    })());return;
  }
  if(isOpenLibrary(u)){
