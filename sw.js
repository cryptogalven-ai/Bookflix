const CACHE='bookflix-shell-v14';
...
async function enhancedResponse(response){
  ...
  try{const text=await response.text();const names=['enhancements','learning','product','intelligence','growth','quality','premium'];const scripts=names.map(n=>text.includes('/'+n+'.js')?'':'<script src="/'+n+'.js"></script>').join('');if(!scripts)return new Response(text,{status:response.status,statusText:response.statusText,headers:response.headers});const injected=text.replace('</body>',`${scripts}</body>`);const headers=new Headers(response.headers);headers.delete('content-length');return new Response(injected,{status:response.status,statusText:response.statusText,headers})}catch{return response}
}
...
self.addEventListener('fetch',event=>{const u=new URL(event.request.url);if(event.request.method!=='GET')return;if(u.origin===location.origin){
    // Never cache API responses: search must always reach the Vercel proxy.
    if(isApi(u))return;
