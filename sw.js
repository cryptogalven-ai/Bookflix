...
async function enhancedResponse(response){
  ...
  try{
    ...
    const hasEnh=text.includes('/enhancements.js');
    const hasLearning=text.includes('/learning.js');
    if(hasEnh&&hasLearning)return new Response(text,{status:response.status,statusText:response.statusText,headers:response.headers});
    const scripts=`${hasEnh?'':'<script src="/enhancements.js"></script>'}${hasLearning?'':'<script src="/learning.js"></script>'}`;
...
async function cachePage(cache,url,response){
  const enhanced=await enhancedResponse(response.clone());
  await cache.put(url,enhanced.clone());
  return enhanced;
}
self.addEventListener('install',event=>event.waitUntil((async()=>{
  ...
  for(const url of SHELL){
    try{
      ...
      if(url==='/'||url==='/index.html')await cachePage(c,url,r);else if(r.ok)await c.put(url,r);
    }catch{}
  }
